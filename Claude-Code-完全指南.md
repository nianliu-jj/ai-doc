...
Claude Code 采用分层架构设计，从底层基础设施到顶层用户交互，每一层都有明确的职责边界。这种设计使得系统具有良好的可维护性和可扩展性。
...
│  │                      Presentation Layer                          │   │
│  │  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐       │   │
│  │  │  CLI Parser   │  │   TUI/REPL    │  │ Output Format │       │   │
│  │  │  命令解析器    │  │  交互界面     │  │   输出格式化   │       │   │
│  │  └───────────────┘  └───────────────┘  └───────────────┘       │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                              │                                          │
...
│  │  └─────────────┘  └─────────────┘  └─────────────┘             │   │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐             │   │
│  │  │  Subagent   │  │   Memory    │  │ Permission  │             │   │
│  │  │  子代理系统  │  │   System    │  │   Manager   │             │   │
│  │  │             │  │   记忆系统   │  │   权限管理   │             │   │
│  │  └─────────────┘  └─────────────┘  └─────────────┘             │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                              │                                          │
...
会话管理器是 Claude Code 的核心组件之一，负责管理用户会话的完整生命周期：
...
impl SessionManager {
    ...
    /// - 加载项目配置
    /// - 恢复历史记忆
    pub async fn create_session(&mut self, working_dir: PathBuf) -> Result<SessionId>;

    /// 恢复会话
    /// - 从存储加载会话状态
    /// - 重建上下文
    /// - 恢复对话历史
    pub async fn resume_session(&mut self, session_id: &SessionId) -> Result<()>;

    /// 压缩会话
    /// - 摘要历史对话
    /// - 保留关键信息
    /// - 释放上下文空间
    pub async fn compact_session(&mut self, session_id: &SessionId) -> Result<()>;
}
```

**Context Manager（上下文管理器）**

上下文管理器负责智能地管理有限的上下文窗口：
...
pub struct ContextManager {
    ...
    /// 内容优先级队列
    priority_queue: PriorityQueue<ContentItem>,
...
impl ToolDispatcher {
    ...
    pub async fn execute(
        ...
        // 1. 触发 PreToolUse 钩子
        let hook_result = self.hook_bus
            .trigger(HookType::PreToolUse, HookContext {
                tool_name: tool_name.to_string(),
                params: params.clone(),
                ..Default::default()
            }).await?;

        // 2. 处理钩子结果（可能修改参数或阻止执行）
        let params = match hook_result {
            HookResult::ModifyParams(new_params) => new_params,
            HookResult::Block(reason) => return Err(ToolError::Blocked(reason)),
            _ => params,
        };
        ...
        // 7. 触发 PostToolUse 钩子
        self.hook_bus.trigger(HookType::PostToolUse, HookContext {
            tool_name: tool_name.to_string(),
            params,
...
#### 24.3 Agent Loop（智能体循环）

Claude Code 的核心是一个称为 "Agent Loop" 的循环机制，这是实现自主任务执行的关键：
...
pub async fn agent_loop(
    ...
    loop {
        ...
        let response = api_client
            .send_message_stream(context)
            .await?;

        // 3. 处理响应
        let (content, tool_calls) = parse_response(response).await?;
        ...
        for tool_call in tool_calls {
            let result = session.execute_tool(&tool_call.name, tool_call.input).await?;
...
└───────────────────────────────────────────────────────────────┘
    ...
    ▼
┌───────────────────────────────────────────────────────────────┐
│  5. Tool Execution（工具执行）                                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐          │
│  │ 参数验证     │  │ 执行工具     │  │ 结果处理    │          │
│  │ Schema 校验  │  │ 沙箱隔离     │  │ 格式化输出  │          │
│  └─────────────┘  └─────────────┘  └─────────────┘          │
└───────────────────────────────────────────────────────────────┘
    │
    ▼
...
Claude Code 的架构设计充分考虑了扩展性：

| 扩展点 | 机制 | 用途 |
|--------|------|------|
| **Tools** | Tool Trait | 添加新的操作能力 |
| **Skills** | Markdown + Frontmatter | 定义可复用的提示词模板 |
| **Hooks** | HookBus + Event | 在关键节点注入自定义逻辑 |
| **MCP** | JSON-RPC Protocol | 集成外部工具和服务 |
| **Subagents** | Agent Trait | 创建专用子代理处理特定任务 |
| **Plugins** | Dynamic Loading | 动态加载扩展模块 |
...
Claude Code 采用多层次压缩策略来应对这些挑战：

**第一层：摘要压缩（Summarization）**
...
impl ConversationSummarizer {
    /// 压缩对话历史
    pub async fn summarize(&self, messages: &[Message]) -> Result<Summary, Error> {
        // 1. 提取关键信息
        let key_points = self.extract_key_points(messages);

        // 2. 生成结构化摘要
        let summary = self.generate_summary(key_points).await?;

        Ok(Summary {
            total_messages: messages.len(),
            compressed_tokens: summary.len(),
            key_decisions: summary.decisions,
            code_changes: summary.changes,
            open_questions: summary.questions,
        })
    }
}
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  会话时间: 2026-04-07 10:30 - 11:45                             │
│  原始消息数: 47 条                                               │
│  原始 Token 数: ~15,000                                         │
│  压缩后 Token 数: ~500                                          │
│                                                                  │
│  【主要任务】                                                     │
│  重构用户认证模块，从 JWT 迁移到 OAuth2                          │
│                                                                  │
│  【关键决策】                                                     │
│  1. 选择 Passport.js 作为 OAuth2 中间件                          │
│  2. 保留现有用户表结构，新增 provider 字段                        │
│  3. 实现 Google 和 GitHub 两种登录方式                           │
│                                                                  │
│  【代码变更】                                                     │
│  - 新增: src/auth/oauth.ts (OAuth2 处理逻辑)                     │
│  - 修改: src/middleware/auth.ts (认证中间件)                     │
│  - 删除: src/auth/jwt.ts (旧 JWT 逻辑)                           │
│                                                                  │
│  【待解决问题】                                                   │
│  - 需要配置生产环境的回调 URL                                    │
│  - 需要添加单元测试                                              │
...
**第二层：层级压缩（Hierarchical Compression）**
...
pub struct TieredCompressor {
    /// 各层级的 Token 预算
    tier_budgets: HashMap<ContentTier, usize>,

    /// 压缩策略配置
    strategies: HashMap<ContentTier, CompressionStrategy>,
}

impl TieredCompressor {
    /// 执行层级压缩
    pub fn compress(&self, context: &mut Context) -> Result<(), Error> {
        let mut total_tokens = 0;
        let max_tokens = context.max_tokens;

        // 从高优先级到低优先级处理
        for tier in [Tier0, Tier1, Tier2, Tier3, Tier4] {
            let budget = self.tier_budgets[tier];
            let items = context.get_items_by_tier(tier);
...
**第三层：智能选择（Intelligent Selection）**
...
impl RelevanceScorer {
    /// 计算内容相关性分数
    pub fn score(&self, content: &ContentItem, query: &str, context: &Context) -> f32 {
        // 1. 关键词匹配分数
        let keyword_score = self.keyword_match(content, query);

        // 2. 语义相似度分数
        let semantic_score = self.semantic_similarity(content, query);

        // 3. 时间衰减分数
        let recency_score = self.recency_factor(content.timestamp);

        // 4. 上下文相关性
        let context_score = self.context_relevance(content, context);

        // 加权综合
        keyword_score * self.keyword_weight +
        semantic_score * self.semantic_weight +
        recency_score * self.recency_weight +
        context_score * (1.0 - self.keyword_weight - self.semantic_weight - self.recency_weight)
    }

    /// 关键词匹配
    fn keyword_match(&self, content: &ContentItem, query: &str) -> f32 {
        let query_terms: HashSet<&str> = query.split_whitespace().collect();
        let content_terms: HashSet<&str> = content.text.split_whitespace().collect();
    ...
    /// 语义相似度（使用嵌入向量）
    fn semantic_similarity(&self, content: &ContentItem, query: &str) -> f32 {
        // 使用预计算的嵌入向量计算余弦相似度
        let query_embedding = self.get_or_compute_embedding(query);
        let content_embedding = self.get_or_compute_embedding(&content.text);

        cosine_similarity(&query_embedding, &content_embedding)
    }

    /// 时间衰减因子
    fn recency_factor(&self, timestamp: DateTime<Utc>) -> f32 {
        let now = Utc::now();
        let age = (now - timestamp).num_minutes() as f32;
...
│  │  Step 3: 层级压缩                                        │    │
│  │  ┌─────────────────────────────────────────────────┐    │    │
│  │  │ Tier 0: 系统提示词 + 工具定义 (15K) [保留]       │    │    │
│  │  │ Tier 1: CLAUDE.md + 当前任务 (3K) [保留]        │    │    │
│  │  │ Tier 2: 高相关文件内容 (40K) [保留]             │    │    │
│  │  │ Tier 3: 对话历史 → 摘要 (2K) [压缩]             │    │    │
│  │  │ Tier 4: 低相关内容 [丢弃]                       │    │    │
│  │  └─────────────────────────────────────────────────┘    │    │
│  └─────────────────────────────────────────────────────────┘    │
│      │ 压缩后: ~60K tokens                                       │
...
## 不推荐
请帮我看看这个文件有什么问题，然后修复它，如果需要的话可以参考其他文件，
记得保持代码风格一致，添加必要的注释...

## 推荐
修复 src/auth/login.ts 第 45 行的类型错误

# 2. 利用 CLAUDE.md 缓存常用信息
...
## 大任务拆分
会话 1: claude -p "分析项目架构，输出架构文档"
会话 2: claude -p "根据架构文档，设计用户模块" architecture.md
会话 3: claude -p "实现用户模块的核心功能" user-module-design.md

# 4. 使用 .claudeignore 排除无关内容
...
```rust
/// 推荐的 Token 预算分配
pub struct RecommendedBudget {
    /// 系统提示词（固定）
    pub system_prompt: 10_000,

    /// 工具定义（固定）
    pub tool_definitions: 5_000,
    ...
    /// 对话历史（压缩后）
    pub conversation_history: 10_000,

    /// 文件内容（动态）
    pub file_content: 80_000,

    /// 输出预留
    pub output_reserve: 32_000,
